...
where EmployeeID in (select physician from undergoes u 
                     where not exists (select * from trained_in 
                                       where Treatment = procedures and physician = u.physician) );
                                       
select a.Name as physician, b.Name as procedures, d.Name as patient ,c.DateUndergoes from physician a, undergoes c, patient d, procedures b
where c.patient = d.SSN
and c.Procedures = b.Code
and c.physician = a.EmployeeID
and not exists (select * from trained_in e
                where e.Treatment = c.procedures
                and e.physician = c.physician );
                
select Name from physician 
where EmployeeID in ( select physician from undergoes a 
                      where DateUndergoes > ( select CertificationExpires from trained_in b
                                              where b.physician = a.physician 
									          and b.treatment = a.procedures));
                                              
select a.Name as physician, b.Name as procedures, c.DateUndergoes, d.Name as patient, e.CertificationExpires from physician a, undergoes c, patient d, procedures b, trained_in e
where c.patient = d.SSN
and c.procedures = b.Code
and c.physician = a.EmployeeID
and b.Code = e.treatment
and a.EmployeeID = e.physician
and c.DateUndergoes > e.CertificationExpires;

select a.Name as patient, b.Name as physician, c.Name as nurse, d.Start, d.End, d.ExaminationRoom, e.Name as PCP
from patient a, physician b, physician e, appointment d left join nurse c on d.prepNurse = c.EmployeeID
where d.patient = a.SSN
and d.physician = b.EmployeeID
and a.PCP = e.EmployeeID
and d.physician <> a.PCP;

select * from undergoes a
where patient <> ( select patient from stay b
                   where a.stay = b.stayID );

select a.Name from nurse a
where EmployeeID in ( select b.nurse from on_call b, Room c
					where b.BlockFloor = c.BlockFloor
                    and b.BlockCode = c.BlockCode
                    and c.RoomNumber = 123 );
